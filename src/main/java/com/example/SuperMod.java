...
public class SuperMod implements ModInitializer, ClientModInitializer {
    ...
    public static class VisualScreen extends Screen {
        ...
        @Override
        protected void init() {
            int x = width / 2 - 100; int y = 30;
            addDrawableChild(ButtonWidget.builder(st("§6♻ AutoSwap", cfg.autoSwap), b -> { cfg.autoSwap = !cfg.autoSwap; cfg.save(); b.setMessage(st("§6♻ AutoSwap", cfg.autoSwap)); }).dimensions(x, y, 200, 20).build());
            addDrawableChild(ButtonWidget.builder(st("§b🛡 Armor HUD", cfg.armorHud), b -> { cfg.armorHud = !cfg.armorHud; cfg.save(); b.setMessage(st("§b🛡 Armor HUD", cfg.armorHud)); }).dimensions(x, y+22, 200, 20).build());
            addDrawableChild(ButtonWidget.builder(st("§f🎯 Crosshair", cfg.crosshair), b -> { cfg.crosshair = !cfg.crosshair; cfg.save(); b.setMessage(st("§f🎯 Crosshair", cfg.crosshair)); }).dimensions(x, y+44, 200, 20).build());
            addDrawableChild(ButtonWidget.builder(Text.of("§6🎨 Цвет: " + COLOR_NAMES[cfg.colorIndex]), b -> { cfg.colorIndex = (cfg.colorIndex + 1) % COLORS.length; cfg.save(); b.setMessage(Text.of("§6🎨 Цвет: " + COLOR_NAMES[cfg.colorIndex])); }).dimensions(x, y+66, 200, 20).build());
            addDrawableChild(ButtonWidget.builder(st("§f🌫 NoFog", cfg.noFog), b -> { cfg.noFog = !cfg.noFog; cfg.save(); b.setMessage(st("§f🌫 NoFog", cfg.noFog)); }).dimensions(x, y+88, 200, 20).build());
            addDrawableChild(ButtonWidget.builder(Text.of("§7Закрыть"), b -> this.close()).dimensions(x, height - 30, 200, 20).build());
        }
        @Override public void render(DrawContext c, int mx, int my, float d) { super.render(c, mx, my, d); }
    }

    public static class Config {
        public boolean noFog = false, autoSwap = true, armorHud = true, crosshair = true;
